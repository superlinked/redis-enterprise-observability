...
# Provider for the domains project
provider "google" {
  alias   = "domains"
...
# Provider for the infrastructure project
provider "google" {
...
resource "google_compute_instance" "redispeerr-vm" {
  ...
  metadata = {
    ...
    ssh-keys = "${var.gcloud_username}:${file("${var.ssh_key_file}.pub")}"
  }
}
...
resource "null_resource" "setup_nginx_ssl" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "server {",
      "    listen 80;",
      "    server_name ${var.subdomain}.${var.domain_name};",
      "    location / {",
      ...
      "sudo certbot --nginx -d ${var.subdomain}.${var.domain_name} --agree-tos --non-interactive --email admin@${var.domain_name}",
